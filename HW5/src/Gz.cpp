...
void Gz::end() {
	...
	if (get(GZ_TEXTURE)) {
		// Apply texture mapping without lighting
		// Only implement the triangle rasterization
		...
		while ((vertexQueue.size() >= 3) && (texCoordQueue.size() >= 3)) {
			...
			for (int i = 0; i < 3; i++) {
				...
				t[i] = texCoordQueue.front(); texCoordQueue.pop();
			}
			frameBuffer.drawTriangleWTexture(v, t, status);
		}
	}
